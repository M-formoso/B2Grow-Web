...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  className,
  logoUrl = '/src/assets/b2grow-logo.png',
  menuButtonColor = '#fff',
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (preContainer) {
        ...
        if (preLayerElsRef.current.length) {
          const offscreen = position === 'left' ? -100 : 100;
          preLayerElsRef.current.forEach((el, i) => {
            const delay = (preLayerElsRef.current.length - 1 - i) * 12;
            gsap.set(el, { xPercent: offscreen + delay });
          });
        }
      }
              ...
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <Link
                    ...
                    data-index={idx + 1}
                    onClick={(e) => {
                      // Prevenir clicks si el menú no está abierto
                      if (!open) {
                        e.preventDefault();
                        e.stopPropagation();
                      }
