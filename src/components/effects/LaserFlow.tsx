...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Fade effect
      if (hasFadedRef.current && fade < 1) {
        fade = Math.min(1, fade + dt * 2);
        uniforms.uFade.value = fade;
      }

      // Mouse smoothing
      const smoothFactor = Math.exp(-dt / Math.max(mouseSmoothTime, 0.001));
      mouseSmooth.lerp(mouseTarget, 1 - smoothFactor);
      uniforms.iMouse.value.set(mouseSmooth.x, mouseSmooth.y, 0, 0);

      // Time updates
      uniforms.iTime.value = time;
      uniforms.uFlowTime.value = time;
      uniforms.uFogTime.value = time;
  ...
  }, [color, horizontalBeamOffset, verticalBeamOffset, wispDensity, mouseTiltStrength, flowSpeed, verticalSizing, horizontalSizing, fogIntensity, fogScale, wispSpeed, wispIntensity, flowStrength, decay, falloffStart, fogFallSpeed, dpr, mouseSmoothTime]);
